...
use crate::autonomi::exit_code::{self, ExitCodeError, INVALID_INPUT_EXIT_CODE, IO_ERROR};
use autonomi::{
    chunk::DataMapChunk,
    client::{
        analyze::Analysis,
...
pub async fn download(addr: &str, dest_path: &str, client: &Client) -> Result<(), ExitCodeError> {
    ...
    let try_private_address =
        crate::autonomi::access::user_data::get_local_private_archive_access(addr).ok();
...
async fn download_priv_archive_to_disk(
    ...
    match last_error {
        ...
        None => {
            println!("Successfully downloaded private data error with local address: {addr}");
            Ok(())
        }
    }
}
...
async fn download_public(
    ...
    let archive = match client.archive_get_public(&address).await {
        ...
        Err(GetError::Deserialization(_)) => {
            println!("Failed to deserialize Public Archive from address {addr}, trying to fetch data assuming it is a single file instead");
...
async fn download_from_datamap(
    ...
    match client.analyze_address(&datamap.to_hex(), false).await {
        ...
        Ok(Analysis::PublicArchive { archive, .. }) => {
            println!("Detected public archive at: {addr}");
            download_pub_archive_to_disk(addr, archive, dest_path, client).await
        }
        Ok(Analysis::PrivateArchive(private_archive)) => {
            println!("Detected private archive at: {addr}");
