...
/// Multipart form upload of a single file
///
/// Example form:
/// ```
...
async fn put_file_public(client: &DwebClient, file: &mut TempFile, tries: u32) -> PutResult {
    ...
    let content_len = match file.file.read_to_end(&mut content) {
        ...
        Err(e) => {
            let message =
                format!("data_put_public() failed to read file '{file_name}' into buffer - {e}");
    ...
    match result {
        ...
        Err(e) => {
            let message =
                format!("data_put_public() failed store file '{file_name}' on the network - {e}");
            println!("DEBUG {message}");
